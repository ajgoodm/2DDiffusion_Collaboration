...
{												// declare variables for user input.
	...
	string Rsp="_R";
	string Rstr=to_string(Radius).substr(0,5);
	string Nsp="_Nstart";
	string Nstr=to_string(NXSTART);
	string Ifstr="Info_";
	
	file1=Nfstr+Dsp+Dstr+Rsp+Rstr+Nsp+Nstr;
	file2=Hfstr+Dsp+Dstr+Rsp+Rstr+Nsp+Nstr;
	file3=Ifstr+Dsp+Dstr+Rsp+Rstr+Nsp+Nstr;
	...
	const int NTRIALS=150;					// number of simulation trials
	...
	const int NBINS=400;					// number of bins to mesh 1um with for 
											// histogram
	...
	hist.resize(NBINS, vector<double>(static_cast<int>(floor(tsteps/100))));
	...
	for (ii=0; ii<NBINS; ii++)
	{
		for (jj=1; jj<floor(tsteps/100); jj++)
		{
			histFile << ", " << hist[ii][jj];
		}
		histFile << endl;
...
int annihilate(double x[][3], int Nexcitons, double radius, vector < vector<double> > &hist, int tindex, double dhist, int NBINS)
{
	...
	for (ii=0; ii<Nexcitons; ii++)
	{
		for (jj=ii+1; jj<Nexcitons; jj++)
		{
			...
			if ((tindex%100==0)&&(int(floor(dist/dhist))<NBINS)&&(dist>2*radius))	//If we've taken n*100 tstepts append hist.
			{																	  	//increment value in histogram bin by 1.
				hist[int(floor(dist/dhist))][tindex/100]=hist[int(floor(dist/dhist))][tindex/100]+1;
			}
